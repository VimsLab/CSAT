...
#   image = np.array(Image.open(requests.get(image_url, stream=True).raw).resize((496,220)))
  image = np.array(Image.open('35_L_1024_original.jpg').convert(mode='RGB'))
...
#   image = draw_boxes(boxes, labels, classes, image, COLORS)
  ...
  atn = stuff(COLORS, model, labels, boxes, input_tensor, image_float_np, classes)
  return image, atn


image, atn = pre()
# Show the image:
plt.imsave('filelike.png', image)
plt.imsave('atnlike.png', atn)
